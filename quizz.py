...
def charger_drapeaux_et_initialiser_session():
    """Charge la liste des drapeaux et initialise l'état de la session Streamlit."""
    
    # Charger la liste des drapeaux uniquement si elle n'est pas déjà dans l'état de session
...
def passer_au_suivant():
    ...
    # Si nous étions en mode réponse, nous passons à la question suivante
...
def application_quiz():
    ...
    if st.session_state['quiz_termine']:
        ...
        if st.button("Recommencer le Quiz"):
            # Supprimer l'état de session pour relancer l'initialisation
            del st.session_state['drapeaux_restants']
            st.rerun()
        return

    # --- Affichage du Drapeau Actuel ---
    drapeaux = st.session_state['drapeaux_originaux']
    index = st.session_state['index_courant']
    drapeau_actuel = drapeaux[index]
    
    # Affichage du titre/progression
    st.markdown(f"**Drapeau n° {index + 1}** sur **{len(drapeaux)}**")
    
    # Affichage du drapeau dans une colonne centrale (Streamlit gère l'affichage SVG)
    col1, col2, col3 = st.columns([1, 4, 1])
    
    with col2:
        # Utilisation de st.image pour afficher le fichier de drapeau
        # 'use_column_width=True' pour s'adapter à la colonne
        st.image(drapeau_actuel['chemin'], use_column_width=True, caption="Quel est ce pays ?")
    
    # --- Affichage de la Réponse ---
    if st.session_state['montrer_reponse']:
        st.markdown(f"<h1 style='text-align: center; color: green;'>{drapeau_actuel['nom_pays']}</h1>", unsafe_allow_html=True)
        # Le bouton s'appelle "Drapeau Suivant"
        st.button("Drapeau Suivant (⏎)", on_click=passer_au_suivant, use_container_width=True)
    else:
        # Le bouton s'appelle "Révéler la Réponse"
